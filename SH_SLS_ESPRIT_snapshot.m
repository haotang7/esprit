...
azimuth=[50;80];      %方位角[42;149],[60;128] 
elevation=[30;49];   %俯仰角 [55;78],[49;70]  
...
for ii=1:length(snapshot)
    ss=randn(I,snapshot(ii))+j*randn(I,snapshot(ii));
    refp=10.^(snr/20);
    S=refp.*ss;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%      Rs=cov(S');
...
for p=1:monte_num
         ...
         noise=randn(M,snapshot(ii))+j*randn(M,snapshot(ii));
...
%         Ka=sqrt(I/(3*25*9));
%         Z=[Z11,Z12;zeros(25*I,I*I),Ka*eye(25*I)];
%         delta_Usk=zeros(size(Sspace_r1));
%         b_re=[vec(R);Ka*vec(delta_Usk)];
%         delta_PsiU=-inv(Z'*Z)*Z'*b_re;
%         delta_Psi=delta_PsiU(1:I*I);
%         FAI_sls=FAI_r1+reshape(delta_Psi,[I,I]);
      ...
      azim_estimated_sls_c(:,p)=angle(diag(Dm_sls_a1_c))*180/pi;
...
figure;
semilogy(snapshot,RMSE_elev_E_r1,'r-o',snapshot,RMSE_elev_SLS_E_r1,'b-^',snapshot,RMSE_elev_E_c,'m-s',snapshot,RMSE_elev_SLS_E_c,'c-h');
legend('RV-SHESPRIT','SLS-RV-SHESPRIT','D-SHESPRIT','SLS-D-SHESPRIT');
xlabel('Snapshot');
ylabel('RMSE of elevation (degree)');

figure;
plot(snapshot,RMSE_azim_E_c,'m-s',snapshot,RMSE_azim_SLS_E_c,'c-h');
legend('D-SHESPRIT','SLS-D-SHESPRIT');
xlabel('SNR (dB)');
ylabel('RMSE of azimuth (degree)');

% figure;
% plot(snr,RMSE_elev_E,'r-o',snr,RMSE_elev_E_r1,'b-^');
% legend('SHESPRIT','RV-SHESPRIT');
% xlabel('SNR (dB)');
% ylabel('RMSE of elevation (degree)');
% 
% figure;
% plot(snr,RMSE_azim_E,'r-o');
% legend('SHESPRIT');
% xlabel('SNR (dB)');
% ylabel('RMSE of azimuth (degree)');
